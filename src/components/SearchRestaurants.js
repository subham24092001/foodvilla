...
const SearchRestaurants = () => {
  ...
  const getSearchItemFoods = async () => {
    const data = await fetch(
      "https://corsproxy.io/?https://www.swiggy.com/dapi/landing/PRE_SEARCH?lat=12.9715987&lng=77.5945627"
    );
  ...
  const getNameFromPopularCusines = (entityId) => {
    // Find the index of '=' and add 1 to get the starting index.
    const startIndex = entityId.indexOf("=") + 1;
    // The ending index is the length of the string.
  ...
  return (
    <>
      ...
      <div className="mx-12 sm:mx-20 md:mx-28 lg:mx-44 xl:mx-60 pt-3 pl-2">
        {searchText === "" ? (
          <div>
            <h1 className="font-extrabold text-[1.43rem] text-[#3d4152]">
              Popular Cuisines
            </h1>
        ...
        {searchText === "" || searchName === searchText ? (
          ...
          <ul>
            {suggestions.map((suggestion, index) => (
              <li key={index}>
                <Link
                  onClick={() => handleClick(suggestion.text, suggestion.type)}
                  to={
                    "/search?query=" +
                    encodeURIComponent(suggestion.text).replace(/%20/g, "+")
                  }
                >
          ...
          <ul>
            <div className="flex gap-2 text-sm font-medium mb-1">
              <h1
                className={
                  isDish === "RESTAURANT"
                    ? "bg-black text-white py-2 px-4 rounded-3xl"
                    : "p-2 border rounded-3xl"
                }
              >
                Restaurants
              </h1>
              ...
              {isDish === "RESTAURANT" && searchByRes !== undefined ? (
                <div>
                  <div className="w-full md:w-1/2">
                    {searchByRes.cards &&
                    searchByRes.cards[0]?.card?.card?.info ? (
                      <SearchResultResCard
                        data={searchByRes.cards[0].card.card.info}
                      />
                    ) : (
                      ""
                    )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 pt-4">
                    {searchByRes.cards &&
                      searchByRes.cards[1]?.card?.card?.restaurants?.map(
                        (res) => (
                    ...
                    {searchByRes.cards &&
                      searchByRes.cards
                        ...
                        .map((res) => (
                          <div
                            className="pr-2 pb-2"
                            key={res?.card?.card?.restaurant?.info?.id}
                          >
                            <SearchResultResCard
                              data={res?.card?.card?.restaurant?.info}
                            />
                          </div>
                        ))}
                  </div>
